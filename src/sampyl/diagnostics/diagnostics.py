...
def compute_r_hat(theta_chains):
    ...
    B_over_n = 1.0 / (m - 1) * np.sum((chain_means - grand_mean) ** 2)

    # compute within sequence variance
    W = 1.0 / (m * (n - 1)) * np.sum((theta_chains.T - chain_means) ** 2)

    # compute pooled esitmate
    var_plus = (n - 1.0) / n * W + B_over_n

    # compute estimate scale reduction factor
    R_hat = (m + 1.0) / m * var_plus / W - (n - 1.0) / (m * n)
...
def compute_n_eff_acf(theta_chain):
    ...
    return n / (1.0 + 2 * stattools.acf(theta_chain)[1:].sum())
